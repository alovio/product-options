...
$GLOBALS['clpo_fail'] = false;

function clpo_check( bool $cond, string $msg ): void {
	echo ( $cond ? 'PASS' : 'FAIL' ) . " - {$msg}\n";
	if ( ! $cond ) {
		$GLOBALS['clpo_fail'] = true;
	}
}
...
$pid = $product->save();
clpo_check( $pid > 0, "created product #{$pid}" );
...
clpo_check( count( $saved['fields'] ) === 2, 'unknown type dropped on save (got ' . count( $saved['fields'] ) . ')' );

// 3. Round-trip read.
$read = $repo->get( $pid );
clpo_check( count( $read['fields'] ) === 2, 'round-trip read 2 fields' );
clpo_check( ( $read['fields'][1]['condition']['field'] ?? '' ) === 'engrave', 'condition preserved through storage' );

// 4. Price calculation (real WC decimals).
$decimals = wc_get_price_decimals();
$total    = PriceCalculator::addon_total( $read, array( 'engrave' => 'yes', 'text' => 'Hi' ), $decimals );
clpo_check( abs( $total - 5.0 ) < 0.001, "addon total = 5.00 when engaged (got {$total})" );
$total0 = PriceCalculator::addon_total( $read, array(), $decimals );
clpo_check( abs( $total0 - 0.0 ) < 0.001, "addon total = 0.00 when not engaged (got {$total0})" );

// 5. Conditional validation.
$err_on  = Validator::validate( $read, array( 'engrave' => 'yes' ) ); // text active + required + empty.
clpo_check( count( $err_on ) === 1, 'required text errors when engrave is on (got ' . count( $err_on ) . ')' );
$err_off = Validator::validate( $read, array( 'engrave' => '' ) ); // text hidden.
clpo_check( count( $err_off ) === 0, 'no error when text hidden (got ' . count( $err_off ) . ')' );

// 6. Sanitization.
$opts = OptionSanitizer::sanitize( $read, array( 'engrave' => '1', 'text' => 'Hello' ) );
clpo_check( ( $opts['engrave'] ?? '' ) === 'yes', 'checkbox normalized to "yes"' );
clpo_check( ( $opts['text'] ?? '' ) === 'Hello', 'text value sanitized + kept' );

// Cleanup.
wp_delete_post( $pid, true );

echo $GLOBALS['clpo_fail'] ? "\nRESULT: FAIL\n" : "\nRESULT: ALL PASS\n";
